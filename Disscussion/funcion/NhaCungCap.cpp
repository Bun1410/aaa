...
NhaCungCap::NhaCungCap(string maNCC ,string tenNCC, string tenSpNhap, string soLuong, string giaSanPham, string soDienThoai)
...
void NhaCungCap::suaNCC(){
    ...
    for (int i = 0; i < listNCC.size(); i++){
        if (listNCC[i].maNCC == maNCC){
            ...
            cout << "Nhap gia san pham: ";
            getline(cin, listNCC[i].giaSanPham);
            cout << "Nhap so dien thoai: ";
...
void NhaCungCap :: hienThi(const int &i){
    cout << "|  " << i+1 ;
        ...
        for(int  j=0; j<13-listNCC[i].giaSanPham.length(); j++){
            cout << " ";
        }
